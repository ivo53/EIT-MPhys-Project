...
def find_all_distances(rec, h_mat, el_coords, n_el, a = 2., npix = 64, cutoff = 0.8):
	...
	#print(el_coords)
	all_meas_pairs = cp.asnumpy(volt_mat(n_el))
...
def simulateMeasurements(fileJac, anomaly=0, measurements=None, v_meas=None, n_el=20, n_per_el=3, n_pix=64, a=2.):
	...
	if v_meas is None:
		...
		f2, raw = fem.solve_eit(volt_mat=meas[ind, 2:], new_ind=new_ind[ind], ex_mat=meas[ind, :2], parser=None, perm=mesh_new['perm'].astype('f8'))
		v_meas = f2.v
		'''
		#plot
		fig = plt.figure(3)
