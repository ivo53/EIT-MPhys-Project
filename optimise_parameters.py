...
class OptimiseMeasurementAlgorithm(object):
	def __init__(self, n_samples=2e4, ne=20, a=2., npix=64, n_per_el=3):
		...
		_, self.el_coords = meshing.fix_electrodes_multiple(centre=None, edgeX=0.1, edgeY=0.1, a=self.a, b=self.a, ppl=self.ne, el_width=0.2, num_per_el=self.n_per_el)
...
result = gp_minimize(opt.objective_function, bounds, x0=initial,
					acq_func = "gp_hedge", n_calls=150, n_random_starts=10, verbose=True, noise=1e-3, n_jobs=-1, model_queue_size=3)
