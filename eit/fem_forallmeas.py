...
class Forward(object):
    ...
    def solve_eit(self, ex_mat=None, step=1, perm=None, parser=None):
        ...
        if ex_mat is None:
            ex_mat = cp.array(eit_scan_lines(20, 8))
        else:
            ex_mat = cp.array(ex_mat)

        if type(step) is int:
            step_arr = step * cp.ones(20)
    ...
    def solveAnomaly(self, ex_mat=None, step=1, perm=None, parser=None):
        ...
        return cp.asnumpy(V), cp.asnumpy(meas), cp.asnumpy(new_ind)
    ...
    def admittanceMatrixE(self):
        shapeParams = self.shapeFunctionParameters()
        whereIsZero = (cp.abs(shapeParams) - 1e-12 < 0)
        indexZero = cp.where(whereIsZero)
        isConst = indexZero[2] == 2 # 1 for const x, 0 for const y
        indicesConstX = cp.where(isConst)[0]
        indicesConstY = cp.where(~isConst)[0]
