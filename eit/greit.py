...
class GREIT(EitBase):
    ...
    def setup(self, method='dist', w=None, p=0.20, lamb=1e-2,
              n=32, s=20., ratio=0.1, w_mat=None):
        """
        set up for GREIT.
    ...
    def _build_dist(self, w_mat):
        ...
        # RM = E[xx^T] / E[yy^T]
        h_mat = np.einsum('jk, lj, lm', w_mat, jac, jac_inv, optimize='optimal')
        #print('pass')
        #h_mat = np.dot(np.dot(w_mat.T, jac.T), jac_inv)
        #print('arrays are equal?',np.sum(np.equal(np.round_(h2_mat, 4), np.round_(h_mat, 4))))
        return h_mat
